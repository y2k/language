...
let fix_java_filename path =
  let name = Filename.basename path in
  let name = String.capitalize_ascii name in
  Filename.concat (Filename.dirname path) name

let make_build_script path =
  let result =
    reduce_files path [] (fun acc file ->
        if filter_source_file file then file :: acc else acc)
  in
  (* print_endline path; *)
  let prelude_path = path ^ "/vendor/prelude/java/src/prelude.clj" in
  ...
  |> List.map (fun file ->
         let target_file =
           Filename.concat path
             (Filename.concat ".github/bin"
                (change_extension (fix_java_filename file) ".java"))
         in
         let create_dir =
