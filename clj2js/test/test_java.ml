...
let assert1 pos code expected =
  let inner_assert () =
    let actual = compile_code code in
    let start = 0 in
...
let assert_file p filename =
  ...
  let expected =
    try In_channel.(with_open_bin (path ^ ".java") input_all)
    with _ ->
      Out_channel.with_open_bin (path ^ ".java") (fun o ->
          Out_channel.output_string o (compile_code code));
      In_channel.(with_open_bin (path ^ ".java") input_all)
  in
...
let assert_strings =
  [
    ...
    assert1 __POS__ "(class foo.bar.Baz)" "foo.bar.Baz.class";
    ...
    assert1 __POS__ {|(defn c [] 3) (defn d [] 4)|}
      {|class Main_shared{public static Object c(){return 3;}public static Object d(){return 4;}}|};
    assert1 __POS__
      {|(defn c [] 3) (comment (defn a [] 1) (defn b [] 2)) (defn d [] 4)|}
      {|class Main_shared{public static Object c(){return 3;}public static Object d(){return 4;}}|};
    assert1 __POS__ {|(defn a [] 1) (defn b [] 2)|}
      {|class Main_shared{public static Object a(){return 1;}public static Object b(){return 2;}}|};
...
|}
    ...
    assert1 __POS__ {|(do (str) (println))|}
