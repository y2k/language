  ...
  let args2, let_args2 =
    ...
    |> List.fold_left
         (fun (args, lets) a ->
           ...
           | node ->
               let virt_arg =
                 SAtom (meta_empty, NameGenerator.get_new_var ())
               in
  ...
  let rec loop let_args =
    ...
    *)
    ...
    | ((SList (_, SAtom (_, "vector") :: _) as a), fn_v) :: t ->
        let virt_arg = SAtom (meta_empty, NameGenerator.get_new_var ()) in
        ...
        let rec loop2 = function
          ...
          | av :: SAtom (_, k) :: t ->
              ( av,
                expand_core_macro2
                  (SList
                     (m, [ SAtom (meta_empty, "get"); v; SAtom (meta_empty, k) ]))
              )
              :: loop2 t
...
let invoke simplify_node = function
  | SList (mfn, SAtom (l, "fn") :: SList (_, _ :: args) :: body) ->
      (let result =
         match desugar_fn_arguments simplify_node args with
         | new_args, [] ->
             SList (mfn, [ SAtom (l, "fn*"); SList (meta_empty, new_args) ])
         | new_args, let_args ->
             SList
               ( mfn,
                 [
                   SAtom (l, "fn*");
                   SList (meta_empty, new_args);
                   SList
                     ( meta_empty,
                       [
                         SAtom (meta_empty, "let*"); SList (meta_empty, let_args);
                       ] );
                 ] )
       in
