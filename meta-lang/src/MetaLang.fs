...
type Node =
    ...
    | Let of (string * Node) list * (Node list)
...
type ExtNode =
    ...
    | ExtLet of (string * ExtNode) list * (ExtNode list)
...
let rec mapToCoreLang =
    ...
    | ExtLet (xs, body) -> Let(xs |> List.map (fun (k, v) -> k, mapToCoreLang v), body |> List.map mapToCoreLang)
