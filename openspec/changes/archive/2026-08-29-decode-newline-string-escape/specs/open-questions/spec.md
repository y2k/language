...
- **GIVEN** quoted strings decode `\n` to LF as specified by `frontend-syntax`
- **AND** other backslash escape pairs remain without accepted language-level semantics
- **WHEN** an escape sequence other than `\n` is parsed, emitted, or evaluated
- **THEN** its intended language-level semantics remain unclear beyond current implementation behavior
