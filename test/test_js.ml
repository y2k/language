...
let tests =
  [
    ...
    ( __POS__,
      ...
      "42" );
    (__POS__, {|(defn test [] (last [1 2 42]))|}, "42");
    ( __POS__,
      {|(defn test [] (+ (get (conj [1 2] 40) 2) (count (conj [1 2] 4))))|},
      "43" );
    (__POS__, {|(defn test [] (if (fn? (fn [] 0)) 42 0))|}, "42");
    (__POS__, {|(defn test [] (if (= __LOC__ "main.clj:1:22") 42 0))|}, "42");
    ( __POS__,
      {|(defn f [a] (fn [] (if (= "/cat" (:b a)) 42 3))) (defn test [] ((f {:b "/cat"})))|},
      "42" );
    (__POS__, {|(defn test [] (first [42 1 2]))|}, "42");
    ( __POS__,
    ...
    ( __POS__,
      ...
      "42" );
    (__POS__, {|(defn test [] (reduce (fn [a b] (+ a b)) 0 [10 20 12]))|}, "42");
    ( __POS__,
      {|(defn test [] (if (and (nil? nil) (number? 3) (boolean? false) (string? "") (vector? [1 2 3])) 42 0))|},
      "42" );
    (__POS__, {|(defn test [] (:b (assoc {:a 1 :b 2 :c 3} :b 42)) )|}, "42");
    (__POS__, {|(defn test [] (count (concat [1 2] [1] [1 2 3])) )|}, "6");
    (__POS__, {|(defn test [] (+ ( * 2 100) (- 50 10) (+ 1 2 2)))|}, "245");
    (__POS__, {|(defn f [] (FIXME "A" 2 "C")) (defn test [] 42)|}, "42");
    ( __POS__,
    ...
    ( __POS__,
      ...
      "42" );
    (__POS__, {|(defn- f [x] x) (defn test [] (f 42))|}, "42");
    (__POS__, {|(defn test [] (if (not false) 42 0))|}, "42");
    ( __POS__,
    ...
    ( __POS__,
      ...
      "42" );
    (__POS__, {|(defn test [] (parseInt (.toString (Number. 42))))|}, "42");
    (__POS__, {|(defn test [] (:b {:a 1 :b 42 :c 3}))|}, "42");
    ( __POS__,
      {|(defn test [] (+ (if (empty? []) 40 0) (if (empty? [1]) 0 2)))|},
      "42" );
    (__POS__, {|(defn test [] (parseInt (.toString "42")))|}, "42");
    ( __POS__,
      {|(defn test [] (let [a (atom 2)] (reset! a 42) (deref a)))|},
      "42" );
    (__POS__, {|(defn test [] (parseInt (str "4" :2)))|}, "42");
    (__POS__, {|(defn test [] (+ (if true 21 1) (if false 1 21)))|}, "42");
    (__POS__, {|(defn test [] (let [x 42] x))|}, "42");
    ( __POS__,
